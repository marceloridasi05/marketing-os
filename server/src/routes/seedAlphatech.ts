/**
 * POST /api/seed/alphatech
 * One-shot seed endpoint — populates AlphaTech mock dataset on the production DB.
 * Protected by SEED_TOKEN env var (or open if not set, for convenience on private deployments).
 */
...
router.post('/', async (req, res) => {
  // Optional token protection — set SEED_TOKEN env var to require it
  ...
  try {
    ...
    // ── Performance Entries (monthly) — acquisition UP, CVR DOWN ─────────
    await db.insert(performanceEntries).values([
      ...
      // Apr 2026 — CVR 1.8% (bottleneck fully visible)
    ...
    // ── Ads KPIs (weekly) — impressions UP, GA CVR DOWN ───────────────────
    const kpiWeeks = [
    ...
    // ── Plan Schedule ─────────────────────────────────────────────────────
    const planRows = [
      { objective: 'Aquisição', action: 'Expansão LinkedIn Ads',       year: 2026, month: 2, value: 'Segmentação enterprise por cargo e setor',                  status: 'ongoing'  },
      ...
      { objective: 'Aquisição', action: 'Expansão LinkedIn Ads',       year: 2026, month: 4, value: 'Expansão de targeting + retargeting visitantes',             status: 'planned'  },
      { objective: 'Aquisição', action: 'Expansão LinkedIn Ads',       year: 2026, month: 5, value: 'Conversation Ads para nurturing',                           status: 'planned'  },
      ...
      { objective: 'Conversão', action: 'Otimização de Landing Pages', year: 2026, month: 4, value: 'Prova social: logos + depoimentos',                         status: 'ongoing'  },
      { objective: 'Conversão', action: 'Otimização de Landing Pages', year: 2026, month: 5, value: 'Otimização mobile + Core Web Vitals',                       status: 'planned'  },
      ...
      { objective: 'Conversão', action: 'Qualificação de Leads (ICP)', year: 2026, month: 4, value: 'Ajuste formulários + lead scoring progressivo',             status: 'ongoing'  },
      { objective: 'Conversão', action: 'Qualificação de Leads (ICP)', year: 2026, month: 5, value: 'Integração score HubSpot → critérios MQL→SQL',             status: 'planned'  },
      ...
      { objective: 'Receita',   action: 'Otimização Pipeline CRM',     year: 2026, month: 3, value: 'Automações de follow-up por estágio',                       status: 'done'     },
      { objective: 'Receita',   action: 'Otimização Pipeline CRM',     year: 2026, month: 4, value: 'Dashboard de pipeline + forecast',                         status: 'ongoing'  },
      { objective: 'Receita',   action: 'Otimização Pipeline CRM',     year: 2026, month: 5, value: 'Treinamento do time de vendas',                             status: 'planned'  },
    ];
    for (const r of planRows) await db.insert(planSchedule).values({ siteId: sid, ...r });

    // ── Initiative Meta ───────────────────────────────────────────────────
    await db.insert(initiativeMeta).values([
      { siteId: sid, objective: 'Aquisição', action: 'Expansão LinkedIn Ads',       businessObjective: 'Aumentar volume de leads qualificados de ICP enterprise em 40% até Q3',    metricKey: 'leads',       expectedOutcome: '+40 leads/mês enterprise, CPL < R$400' },
      { siteId: sid, objective: 'Conversão', action: 'Otimização de Landing Pages', businessObjective: 'Aumentar taxa de conversão site→lead de 1.8% para 3.0% até maio',          metricKey: 'sessions',    expectedOutcome: 'CVR 1.8% → 3.0%, CPL de R$250 → R$170' },
      { siteId: sid, objective: 'Conversão', action: 'Qualificação de Leads (ICP)', businessObjective: 'Melhorar taxa lead→oportunidade de 18% para 30% alinhando critérios MQL',  metricKey: 'leads',       expectedOutcome: 'Lead-to-opp 18% → 30%' },
      { siteId: sid, objective: 'Receita',   action: 'Otimização Pipeline CRM',     businessObjective: 'Reduzir ciclo de vendas de 75 para 60 dias e aumentar forecast accuracy', metricKey: 'conversions', expectedOutcome: 'Ciclo 75d → 60d, forecast > 80%' },
    ]);
    ...
    await db.insert(experiments).values([
      {
        siteId: sid,
        hypothesis: 'Segmentando LinkedIn Ads por setor (Financeiro/Seguros), a qualidade dos leads melhora e a taxa lead→oportunidade sobe de 18% para 28%+',
        expectedResult: 'Lead-to-opportunity rate acima de 28% (baseline 18%)',
        duration: '4 semanas', startDate: '2026-04-01', endDate: null,
        channel: 'LinkedIn Ads', metric: 'Lead-to-opportunity rate',
        baselineValue: '18%', resultValue: null, learning: null,
        status: 'running', successful: null, category: 'Conversão',
      },
      {
        siteId: sid,
        hypothesis: 'Reduzindo o formulário de 8 para 5 campos, a taxa de conversão da landing page aumenta',
        expectedResult: 'CVR sobe de 2.1% para 3.5%+',
        duration: '4 semanas', startDate: '2026-03-01', endDate: '2026-03-28',
        channel: 'Site', metric: 'CVR (Landing Page)',
        baselineValue: '2.1%', resultValue: '3.2%',
        learning: 'Formulário curto aumentou CVR em 52%. Aplicar em todas as LPs.',
        status: 'completed', successful: 'yes', category: 'Conversão',
      },
    ]);

    // ── Ideas ─────────────────────────────────────────────────────────────
    await db.insert(ideas).values([
      { siteId: sid, title: 'Webinar: Gestão de Risco em 2026',          description: 'Evento online 1h com depoimento de clientes ao vivo.',  targetDate: '2026-05-20', complexity: 'medium', category: 'Evento',    status: 'planned', priority: 'high'   },
      { siteId: sid, title: 'Calculadora de ROI interativa no site',     description: 'Widget: prospect insere dados → vê estimativa de ROI.', targetDate: '2026-06-01', complexity: 'high',   category: 'Produto',   status: 'planned', priority: 'high'   },
      { siteId: sid, title: 'Case study: Banco Regional + AlphaTech',    description: 'Documentar ROI, tempo de impl. e depoimento do CRO.',   targetDate: null,         complexity: 'high',   category: 'Conteúdo',  status: 'idea',    priority: 'medium' },
      { siteId: sid, title: 'Email nurturing para leads com score baixo', description: 'Fluxo 8 emails / 30 dias para leads score < 40.',       targetDate: '2026-05-15', complexity: 'medium', category: 'Campanha',  status: 'idea',    priority: 'medium' },
    ]);

    // ── Suppliers ─────────────────────────────────────────────────────────
    await db.insert(suppliers).values([
      { siteId: sid, name: 'Agência Performance Digital', category: 'Mídia Paga',      type: 'fornecedor', notes: 'Gestão Google Ads + LinkedIn Ads' },
      { siteId: sid, name: 'HubSpot',                     category: 'CRM e Automação', type: 'tool',       website: 'https://hubspot.com',  notes: 'CRM principal' },
      { siteId: sid, name: 'SEMrush',                     category: 'SEO e Analytics', type: 'tool',       website: 'https://semrush.com',  notes: 'SEO e análise de concorrência' },
    ...
    res.json({
      success: true,
      siteId: sid,
      message: 'AlphaTech seed concluído',
      summary: {
        channels: 4, performanceEntries: 12, adsKpiWeeks: 9,
        siteDataWeeks: 9, budgetItems: 15, adsBudgets: 4,
        goals: 9, planCells: 14, initiativeMeta: 4,
        experiments: 2, ideas: 4, suppliers: 4,
      },
    });
