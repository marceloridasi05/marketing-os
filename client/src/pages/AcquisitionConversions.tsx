...
const CHANNELS = ['google_ads', 'linkedin', 'meta', 'organic', 'direct', 'email', 'referral', 'other'];
...
export default function AcquisitionConversions() {
  ...
  const {
    ...
    isFieldRequired,
    getGuidanceMessage,
  } = useFieldConfiguration(siteId, 'acquisition-conversions');

  const [rawData, setRawData] = useState<AcquisitionRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [lastSync, setLastSync] = useState<string | null>(null);
  const [manualMode, setManualMode] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [editingRow, setEditingRow] = useState<Partial<AcquisitionRow> | null>(null);
  const { timePeriod, dateRange, filterProps } = useTimeFilter('all');

  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const rows = await api.get<AcquisitionRow[]>('/acquisition-conversions');
      setRawData(rows);
    } catch (err) {
  ...
  const handleSaveRow = async () => {
    ...
    try {
      if (editingRow.id) {
        await api.put(`/acquisition-conversions/${editingRow.id}`, editingRow);
      } else {
        await api.post('/acquisition-conversions/manual', editingRow);
      }
  ...
  const handleDeleteRow = async (id: number) => {
    ...
    try {
      await api.delete(`/acquisition-conversions/${id}`);
  ...
  const withData = data.filter(r => r.conversions != null && r.conversions > 0);
  ...
  const rConversions = useColumnRange(data, 'conversions');
  const rImpressions = useColumnRange(data, 'impressions');

  // Channel breakdown
  const channelBreakdown = useMemo(() => {
    const agg: Record<string, { clicks: number; conversions: number; impressions: number }> = {};
    for (const r of withData) {
      const ch = r.channel || 'unknown';
      if (!agg[ch]) agg[ch] = { clicks: 0, conversions: 0, impressions: 0 };
  ...
  return (
    <div>
      <PageHeader
        title="Aquisição & Conversões"
        description="Dados de aquisição por canal e campanha"
        actions={
      ...
      {loading ? (
        ...
        <Card className="p-8 text-center">
          <p className="text-gray-500 mb-4">Nenhum dado de aquisição ainda.</p>
          <button
        ...
        <Card className="p-6 mb-6">
          <h3 className="text-lg font-semibold mb-4">
            {editingRow.id ? 'Editar' : 'Adicionar'} Aquisição
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            {filterField('conversionRate') && (
              <div>
                <label className="text-sm font-medium text-gray-700">
                  Taxa Conversão % {isFieldRequired('conversionRate') && '*'}
                </label>
                <input
                  type="number"
                  placeholder="0.00"
                  step="0.01"
                  value={editingRow.conversionRate || ''}
                  onChange={(e) => setEditingRow({ ...editingRow, conversionRate: parseFloat(e.target.value) || null })}
                  className="mt-1 w-full px-3 py-2 border border-gray-300 rounded-md text-sm"
                />
              </div>
            )}
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
            ...
            <Card className="min-w-0">
              <p className="text-xs font-medium text-gray-500 uppercase">Conversões Totais</p>
              <p className="text-2xl font-semibold text-gray-900 mt-1">{fmtNum(totalConversions)}</p>
            </Card>
            <Card className="min-w-0">
              <p className="text-xs font-medium text-gray-500 uppercase">Taxa Conversão</p>
              <p className="text-2xl font-semibold text-gray-900 mt-1">{avgConversionRate.toFixed(2)}%</p>
            </Card>
            <Card className="min-w-0">
              <p className="text-xs font-medium text-gray-500 uppercase">CTR Médio</p>
              <p className="text-2xl font-semibold text-gray-900 mt-1">{avgCtr.toFixed(2)}%</p>
            </Card>
            <Card className="min-w-0">
              <p className="text-xs font-medium text-gray-500 uppercase">Impressões</p>
              <p className="text-2xl font-semibold text-gray-900 mt-1">{fmtNum(totalImpressions)}</p>
            </Card>
          </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-200">
                      ...
                      <th className="text-center py-2.5 px-2 font-medium text-gray-500">Taxa Conv.</th>
                      <th className="text-center py-2.5 px-2 font-medium text-gray-500">CTR</th>
                    </tr>
                  </thead>
                  <tbody>
                    {channelBreakdown.map((ch) => (
                      <tr key={ch.channel} className="border-b border-gray-100 hover:bg-gray-50">
                        ...
                        {filterField('conversions') && <HeatTd value={ch.conversions} min={rConversions.min} max={rConversions.max} />}
                        {filterField('impressions') && <HeatTd value={ch.impressions} min={rImpressions.min} max={rImpressions.max} />}
                        <td className="py-2 px-2 text-center text-gray-600">{ch.conversionRate.toFixed(2)}%</td>
                        <td className="py-2 px-2 text-center text-gray-600">{ch.ctr.toFixed(2)}%</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {/* Acquisition Table */}
          <CollapsibleCard title="Dados de Aquisição" className="mb-6">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-200">
                    ...
                    {filterField('conversionRate') && <sort.SortHeader k="conversionRate" label={`Taxa %${isFieldRequired('conversionRate') ? ' *' : ''}`} align="right" />}
                ...
                <tbody>
                  {sort.sorted.map((r) => (
                    <tr key={r.id} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      {filterField('conversions') && <HeatTd value={r.conversions} min={rConversions.min} max={rConversions.max} />}
                      {filterField('impressions') && <HeatTd value={r.impressions} min={rImpressions.min} max={rImpressions.max} />}
                      {filterField('conversionRate') && <td className="py-2 px-2 text-center text-gray-600">{r.conversionRate?.toFixed(2) ?? '—'}%</td>}
